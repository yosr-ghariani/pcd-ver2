package PCD.BACKEND.RECRAFTMARKET.service.role;
...
public class RoleService {
    ...
    {
        final Role role = roleRepository.fetchRoleByName(roleName).orElseThrow(
                ()-> new ResourceNotFoundException(String.format("Role with ID %s could not be found.", roleName))
        );
        return new ResponseEntity<>(role,HttpStatus.OK);
    }
    ...
    public ResponseEntity<String> updateRoleById(final long roleId, final @NotNull Role roleDetails)
    {
        ...
        final String successMessage = String.format("Role with name : %s is updated to : %s",role.getName(),updatedRole.getName());
    ...
    public boolean roleExists(String roleName) {
        return roleRepository.fetchRoleByName(roleName).isPresent();
    }
}
